<template>
  <page-shell title="Bot 库" description="BOT机器抓取识别库与分类">
    ...
    <fs-slide-transition :transition-key="activeTab">
        ...
        <template #cell="{ column, record }">
          <template v-if="column.key === 'categories'">
            <div v-if="(record.categories || []).length" class="category-tags">
              <a-tag v-for="cat in record.categories" :key="cat">
                {{ categoryLabel(cat) }}
              </a-tag>
            </div>
            <span v-else class="muted">—</span>
          </template>
          <template v-else-if="column.key === 'ua_patterns'">
            <div v-if="(record.ua_patterns || []).length" class="ua-pattern-tags">
              <a-tag v-for="pattern in record.ua_patterns" :key="pattern" class="ua-pattern-tag">
                {{ pattern }}
              </a-tag>
            </div>
            <span v-else class="muted">—</span>
      ...
      <div v-else-if="activeTab === 'vendored'" class="fs-card vendored-panel">
...
function canDeleteCategory(row: Record<string, any>) {
  return row.value !== "other";
}
