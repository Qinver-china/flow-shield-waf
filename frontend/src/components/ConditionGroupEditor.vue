  ...
  <div :class="['cond-group', { 'cond-group-nested': depth > 0 }]">
    <div class="logic-row">
      ...
      <div v-if="!readonly" class="logic-actions">
        <a-button size="small" type="primary" @click="addLeaf">+ 添加条件</a-button>
        <a-button v-if="canNest" size="small" class="add-group-btn" @click="addGroup">
          + 添加条件组
        </a-button>
        <a-button v-if="depth > 0" danger size="small" type="text" @click="$emit('remove')">
          删除组
        </a-button>
      </div>
    </div>
    ...
    <a-empty
      v-else
      :description="depth === 0 ? '暂无条件（留空表示匹配全部请求）' : '该条件组为空'"
